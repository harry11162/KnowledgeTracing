...
class DKVMNcell(nn.Module):
    def __init__(self, input_size, hidden_size, memory_size, dsize=8, momentum=0.9):
        ...
        self.w2 = torch.Tensor([1 / dsize] * dsize)
        self.momentum = momentum

    def forward(self, x, memory, w2):
        N, C2 = x.size()
        C = C2 // 2
        ...
        w2 = w2 * self.momentum + self.kw2(k) * (1 - self.momentum)
        w2_s = F.softmax(w2, dim=-1)
        ...
        e = torch.sigmoid(self.erase(v, w2_s))
        a = torch.tanh(self.add(v, w2_s))
