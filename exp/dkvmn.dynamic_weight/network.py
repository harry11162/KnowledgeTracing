...
class DynamicLinear(nn.Module):
    def __init__(self, in_features, out_features, dsize):
        ...
        self.expert = nn.Linear(in_features, dsize)
    ...
    def forward(self, input):
        w = F.softmax(self.expert(input), dim=-1)
...
class DKVMNcell(nn.Module):
    def __init__(self, input_size, hidden_size, memory_size):
        ...
        self.km = DynamicLinear(hidden_size, memory_size, dsize=8)
        ...
        self.erase = DynamicLinear(hidden_size, hidden_size, dsize=8)
        self.add = DynamicLinear(hidden_size, hidden_size, dsize=8)
